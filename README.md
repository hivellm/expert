...
**Characteristics**:
- ✅ Experts are **neural network layers** (MLPs) inside the model
- ✅ All experts **trained together** as single model
- ✅ Gating network **learned during training** (automatic selection)
- ✅ Typically activates **1-3 experts per token** (sparse routing)
- ❌ **Cannot add new experts** after training
- ❌ **Cannot distribute experts separately** (monolithic)
- ❌ **Requires 100GB+** to load entire model (all experts in memory)
- ❌ **Expensive to train** (~$1M+ for Mixtral-scale)
...
**Characteristics**:
- ✅ Experts are **external adapters** (LoRA/DoRA/IA³ weights)
- ✅ Each expert **trained independently** (separate datasets)
- ✅ Router is **heuristic/rule-based** (keywords, embeddings, user choice)
- ✅ Activates **1-10 experts per query** (loaded as needed)
- ✅ **Can add new experts anytime** (train and deploy separately)
- ✅ **Distributable via Git** (each expert is ~5-80MB file)
- ✅ **Only loads ~1GB total** (base + needed adapters)
- ✅ **Cheap to train** ($15-50 per expert with synthetic data)
...
→ Specialized translation without 100B parameter model
```
...
└──────────────────────┬──────────────────────────────────────┘
                       ...
                       ▼
        ┌──────────────────────────────┐
        │   Router/Reasoning (CPU)     │◄─── Heuristics + Embeddings
        │   - Classify task            │     + Mini-policy
        │   - Select up to 10 experts  │
        │   - Tune parameters          │
        └──────────────┬───────────────┘
                       │
                       ▼
        ┌──────────────────────────────┐
        │   Expert Loader (SSD→VRAM)   │
        │   - Hot-load adapters        │
        │   - ~1-10ms per expert       │
        └──────────────┬───────────────┘
                       │
        ...
        │  Base: Qwen3-0.6B (INT4)     │◄─── Paged KV cache
        │  + Experts (LoRA/IA³/DoRA)   │     CUDA streams
        └──────────────┬───────────────┘
                       │
                       ▼
                  ┌────────┐
...
- ✅ Generic queries preserve generalist capabilities
- ✅ Domain queries activate correct expert
- ✅ Multi-expert routing works correctly
- ✅ Zero overhead (<0.5ms router latency)
...
- ✅ Expert training CLI (Python + PyTorch/PEFT/TRL)
- ✅ Windows setup automation (CUDA 12.1 + PyTorch 2.5.1)
...
- **Dataset quality**: MySQL→PostgreSQL syntax fixes, validation via sqlglot
...
**Current Status**: ✅ Fully functional training pipeline on Windows + Linux  
**Unsloth Support**: ✅ Optional 2x speedup (set `use_unsloth: true` in manifest)  
**Performance**: 
...
- ✅ **LoRA/DoRA adapter merging** implemented (W' = W + scale × B × A)
- ✅ **Expert hot-loading** from .expert packages
- ✅ **Registry system** for installed experts
- ✅ **CUDA acceleration** with BF16/FP32 support
- ✅ **One-shot inference** mode for scripting
- 🔄 Paged KV cache implementation
- 🔄 Marketplace CLI and verification
- 🔄 gRPC/HTTP API + Node/Python bindings

**Why Rust:** Performance, single binary deployment, memory safety, no GC pauses

**Current Status**: ✅ Functional inference runtime with adapter merging  
**Performance**:
...
### Tested Hardware
- ✅ **NVIDIA RTX 4090** (24GB VRAM) - Full performance, tested
- ✅ **NVIDIA RTX 3060** (8GB VRAM) - Minimum viable
- ✅ **CUDA 12.1** - Fully functional with PyTorch 2.5.1
- ✅ **Windows 11** + **Ubuntu 24.04** (WSL) - Both validated
