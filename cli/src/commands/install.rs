...
use crate::registry::{ExpertRegistry, ExpertEntry, AdapterEntry};
...
fn install_with_deps(source: &str, dev_mode: bool, check_deps: bool, depth: usize) -> Result<(), Error> {
    if depth > 10 {
        return Err(Error::Installation("Dependency depth exceeded 10 (circular dependency?)".to_string()));
    }
    ...
    if !manifest_path.exists() {
        return Err(Error::Manifest("manifest.json not found in source".to_string()));
    }
    
    let manifest = Manifest::load(&manifest_path)?;
    
    // Check if already installed
    if registry.has_expert(&manifest.name) && !dev_mode {
        println!("{}[INFO] Expert '{}' already installed, skipping", indent, manifest.name);
        return Ok(());
    }
    ...
    let install_path = if dev_mode {
        ...
        copy_expert(&temp_dir, &manifest, &registry)?
    };
    ...
    // Update registry
    registry.add_expert(expert_entry);
    registry.save()?;
    
    println!("{}[OK] Expert '{}' installed successfully!", indent, manifest.name);
...
impl InstallSource {
    ...
    fn fetch(&self) -> Result<PathBuf, Error> {
        match self {
            InstallSource::Git { url, ref_spec } => {
                clone_git_repo(url, ref_spec.as_deref())
            }
            ...
            InstallSource::Package(path) => {
                extract_expert_package(path)
            }
        }
    }
...
fn extract_expert_package(package_path: &Path) -> Result<PathBuf, Error> {
    ...
    if !package_path.exists() {
        return Err(Error::Installation(format!("Package not found: {}", package_path.display())));
    }
...
fn check_base_model_compatibility(manifest: &Manifest, registry: &ExpertRegistry) -> Result<(), Error> {
    let required_models = manifest.get_base_models();
    
    if required_models.is_empty() {
        return Err(Error::Manifest("No base models specified in manifest".to_string()));
    }
    
    for model_name in &required_models {
        if !registry.has_base_model(model_name) {
            println!("[WARN] Required base model '{}' not found in registry", model_name);
...
fn copy_expert(source: &Path, manifest: &Manifest, registry: &ExpertRegistry) -> Result<PathBuf, Error> {
    let install_path = registry.install_dir.join(&manifest.name);
    
    if install_path.exists() {
        println!("[WARN] Expert already exists, removing old version...");
...
fn verify_signature(expert_dir: &Path, manifest: &Manifest) -> Result<(), Error> {
    ...
    let pubkey_hex = integrity.pubkey.as_ref()
    ...
    file_hashes.push(("manifest.json".to_string(), compute_sha256_string(&manifest_content)));
    
    // Hash other files (TODO: scan directory for all files)
    file_hashes.sort_by(|a, b| a.0.cmp(&b.0));
    
    let canonical_message: String = file_hashes.iter()
        .map(|(file, hash)| format!("{}:{}", file, hash))
        .collect::<Vec<_>>()
        .join("\n");
    
    // Verify
    if verifying_key.verify(canonical_message.as_bytes(), &signature).is_ok() {
        println!("  [OK] Signature verified!");
        let key_preview = if pubkey_hex.len() > 32 {
            format!("{}...{}", &pubkey_hex[..16], &pubkey_hex[pubkey_hex.len()-16..])
        } else {
            pubkey_hex.clone()
        };
    ...
    for dep in dependencies {
        ...
        if let Some(installed) = current_registry.get_expert(dep_name) {
            if let Some(_version_req) = dep_version {
                ...
                println!("  [OK] Dependency '{}' already installed (v{})", dep_name, installed.version);
            } else {
...
fn create_expert_entry(manifest: &Manifest, install_path: &Path, source: &str) -> Result<ExpertEntry, Error> {
    let base_model = manifest.get_base_models()
        .first()
    ...
    let adapters = if let Some(ref adapters) = manifest.adapters {
        adapters.iter().map(|a| AdapterEntry {
        ...
        models.first()
            .map(|m| m.adapters.iter().map(|a| AdapterEntry {
                adapter_type: a.adapter_type.clone(),
                path: PathBuf::from(&a.path),
                size_bytes: a.size_bytes.unwrap_or(0),
                sha256: a.sha256.clone(),
    ...
    let dependencies: Vec<_> = manifest.constraints.requires.iter().map(|dep: &String| {
        let parts: Vec<&str> = dep.split('@').collect();
