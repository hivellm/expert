...
pub fn package(
    ...
    // Load manifest
    println!("  {} {}", "📄".bright_blue(), "Loading manifest...".bright_white());
    ...
    println!("  {} Schema: {}", "→".bright_blue(), schema_version.as_str().bright_white());
    println!();
    ...
    match schema_version {
        SchemaVersion::V1_0 => {
            if model.is_some() {
                println!("{}", "  ⚠️  --model flag ignored for schema v1.0 (single model only)".bright_yellow());
            }
            package_v1(&manifest, &weights_dir, output, include_tests, list_contents)?;
        }
        SchemaVersion::V2_0 => {
            ...
            let model_name = if let Some(model_arg) = model {
                ...
                let available_models: Vec<String> = manifest.base_models
                    .as_ref()
                ...
                if available_models.len() == 1 {
                    ...
                    println!("  {} Auto-detected model: {}", "→".bright_blue(), auto_model.bright_white());
                    auto_model
            ...
            package_v2(&manifest, &weights_dir, output, &model_name, include_tests, list_contents)?;
        }
    }
...
fn package_v1(
    ...
    println!("  {} {}", "📦".bright_blue(), "Packaging schema v1.0 (single model)...".bright_white());
    ...
    let output_file = output.unwrap_or_else(|| {
        let model_suffix = if let Some(ref base_model) = manifest.base_model {
            ...
            let model_name = base_model.name
                .split('/')
                .last()
        ...
        PathBuf::from(format!("{}{}.v{}.expert", manifest.name, model_suffix, manifest.version))
    });
    
    println!("  {} Output: {}", "→".bright_blue(), output_file.display().to_string().bright_white());
    println!();
    
    println!("  {} {}", "🔨".bright_blue(), "Creating package...".bright_white());
    
    // Create tar.gz archive
    let tar_gz = File::create(&output_file).map_err(|e| {
        Error::Packaging(format!("Failed to create output file: {}", e))
    })?;
    ...
    tar.append_data(&mut header, "manifest.json", manifest_bytes).map_err(|e| {
        Error::Packaging(format!("Failed to add manifest to archive: {}", e))
    })?;
    ...
    if let Some(ref adapters) = manifest.adapters {
        for adapter in adapters {
            ...
            let adapter_relative_path = adapter.path
            ...
            if !weight_path.exists() {
                return Err(Error::Packaging(
                    format!("Adapter path not found: {}", weight_path.display())
                ));
            }
            ...
            if weight_path.is_dir() {
                ...
                for file_name in &essential_files {
                    ...
                    if file_path.exists() {
                        ...
                        tar.append_path_with_name(&file_path, &archive_path).map_err(|e| {
                ...
                println!("    {} {} ({} files)", "✓".bright_green(), adapter.path.bright_white(), added_count);
            } else {
                // Add single file
                tar.append_path_with_name(&weight_path, &adapter.path).map_err(|e| {
                    Error::Packaging(format!("Failed to add adapter file {}: {}", weight_path.display(), e))
                })?;
    ...
    for soft_prompt in &manifest.soft_prompts {
        let prompt_path = weights_dir.parent().unwrap_or(weights_dir).join(&soft_prompt.path);
        if prompt_path.exists() {
            tar.append_path_with_name(&prompt_path, &soft_prompt.path).ok();
            println!("    {} {}", "✓".bright_green(), soft_prompt.path.bright_white());
        } else {
            println!("    {} {} (not found, skipping)", "⚠️ ".bright_yellow(), soft_prompt.path.bright_white());
        }
    }
    ...
    if grammar_path.exists() {
        tar.append_path_with_name(&grammar_path, "grammar.gbnf").ok();
    ...
    if let Some(ref training) = manifest.training.decoding {
        if let Some(ref grammar_file) = training.grammar_file {
            ...
            if grammar_custom_path.exists() {
                tar.append_path_with_name(&grammar_custom_path, grammar_file).ok();
                println!("    {} {} (from manifest)", "✓".bright_green(), grammar_file);
            } else {
                println!("    {} {} (from manifest, not found)", "⚠️ ".bright_yellow(), grammar_file);
            }
        }
    }
    ...
    if include_tests {
        ...
        if tests_dir.exists() && tests_dir.is_dir() {
            ...
            for entry in fs::read_dir(&tests_dir).unwrap_or_else(|_| panic!("Failed to read tests directory")) {
                if let Ok(entry) = entry {
                    let path = entry.path();
                    if path.is_file() {
                        let archive_path = format!("tests/{}", path.file_name().unwrap().to_string_lossy());
    ...
    tar.finish().map_err(|e| {
        Error::Packaging(format!("Failed to finalize archive: {}", e))
    })?;
    ...
    println!();
    println!("  {} Package created successfully!", "✅".bright_green().bold());
    println!("  {} File: {}", "→".bright_blue(), output_file.display().to_string().bright_white());
    println!("  {} Size: {:.2} MB", "→".bright_blue(), size_mb.to_string().bright_white());
    ...
    println!("  {} SHA256: {}", "→".bright_blue(), hash_hex.bright_white());
    
    // Write checksum file
    let checksum_file = output_file.with_extension("expert.sha256");
    let checksum_content = format!("{}  {}\n", hash_hex, output_file.file_name().unwrap().to_string_lossy());
    fs::write(&checksum_file, checksum_content)?;
    println!("  {} Checksum: {}", "→".bright_blue(), checksum_file.display().to_string().bright_white());
...
fn package_v2(
    ...
    println!("  {} {}", "📦".bright_blue(), "Packaging schema v2.0 (multi-model)...".bright_white());
    println!("  {} Model: {}", "→".bright_blue(), model_name.bright_white());
    ...
    if let Some(checkpoint) = packaging_checkpoint {
        println!("  {} Packaging checkpoint: {}", "→".bright_blue(), checkpoint.bright_white());
    }
    ...
    let selected_model = base_models.iter()
    ...
    println!("    {} SHA256: {}", "→".bright_blue(), 
        selected_model.sha256.as_deref().unwrap_or("none").bright_white());
    println!("    {} Quantization: {}", "→".bright_blue(), 
        selected_model.quantization.as_deref().unwrap_or("none").bright_white());
    println!("    {} Adapters: {}", "→".bright_blue(), 
    ...
    if packaging_checkpoint.is_some() {
        for (i, adapter) in selected_model.adapters.iter().enumerate() {
            println!("    {} Adapter {} original path: {}", "→".bright_blue(), 
                i + 1, adapter.path.bright_white());
        }
    }
    ...
    if let Some(checkpoint) = packaging_checkpoint {
        println!("  {} Adjusting adapter paths to use {}", "→".bright_blue(), checkpoint.bright_white());
        for adapter in &mut model_with_adjusted_paths.adapters {
            ...
            if adapter.path.contains("/final") || adapter.path.contains("\\final") {
                ...
                adapter.path = adapter.path.replace("\\final", &format!("\\{}", checkpoint));
            } else if adapter.path.contains("/checkpoint-") || adapter.path.contains("\\checkpoint-") {
    ...
    println!("  {} {}", "📁".bright_blue(), "Package contents:".bright_white());
    println!("    {} Filtered manifest (model: {})", "→".bright_blue(), model_name.bright_white());
    ...
    if !manifest.soft_prompts.is_empty() {
        println!("    {} {} soft prompt(s) (shared)", "→".bright_blue(), 
    ...
    println!("    {} manifest.json (filtered for {})", "✓".bright_green(), model_name.bright_white());
    ...
    for adapter in &model_with_adjusted_paths.adapters {
        ...
        if weight_path.is_dir() {
            ...
            println!("    {} Adapter files ({} files added to root)", "✓".bright_green(), added_count);
        } else {
            // Add single file to root
            let file_name = weight_path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("adapter.bin");
            tar.append_path_with_name(&weight_path, file_name).map_err(|e| {
                Error::Packaging(format!("Failed to add adapter file {}: {}", weight_path.display(), e))
            })?;
            
            println!("    {} {} (added to root)", "✓".bright_green(), file_name.bright_white());
        }
    }
    ...
    if let Some(ref training) = manifest.training.decoding {
        if let Some(ref grammar_file) = training.grammar_file {
            ...
            if grammar_custom_path.exists() {
                ...
                tar.append_path_with_name(&grammar_custom_path, file_name).ok();
                println!("    {} {} (from manifest, added to root)", "✓".bright_green(), file_name);
            } else {
                println!("    {} {} (from manifest, not found)", "⚠️ ".bright_yellow(), grammar_file);
            }
        }
    ...
    if include_tests {
        ...
        if tests_dir.exists() && tests_dir.is_dir() {
            ...
            if test_count > 0 {
                println!("    {} tests/ ({} files, shared)", "✓".bright_green(), test_count);
            }
        } else {
            println!("    {} tests/ directory not found", "⚠️ ".bright_yellow());
        }
