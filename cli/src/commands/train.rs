...
pub fn train(
    ...
    fn resolve_dataset_path(dataset_path: &str, manifest_path: &PathBuf) -> Result<PathBuf> {
        ...
        if dataset_path.contains('/') && !dataset_path.starts_with('.') && !dataset_path.starts_with('/') {
            ...
            let manifest_dir = manifest_path.parent()
                .ok_or_else(|| crate::error::Error::Manifest("Invalid manifest path".to_string()))?;
    ...
    if let Some(epochs) = epochs_override {
        println!("Overriding epochs: {} -> {}", manifest.training.config.epochs, epochs);
    ...
    let is_huggingface = !is_multi_task && dataset_str.contains('/') && !dataset_str.starts_with('.') && !dataset_str.starts_with('/');
    ...
    if base_models.is_empty() {
        ...
        if let Some(ref base) = manifest.base_model {
            println!("  Quantization: {}", base.quantization.as_deref().unwrap_or("none").bright_white());
        } else if let Some(ref models) = manifest.base_models {
            if let Some(model) = models.first() {
                println!("  Quantization: {}", model.quantization.as_deref().unwrap_or("none").bright_white());
            }
        }
    } else {
    ...
    println!();
    println!("  Adapter Type: {}", manifest.training.config.adapter_type.bright_white());
    ...
    println!("  Target Modules: {}", manifest.training.config.target_modules.join(", ").bright_white());
    println!();
    println!("  Epochs: {}", manifest.training.config.epochs.to_string().bright_white());
    println!("  Learning Rate: {}", manifest.training.config.learning_rate.to_string().bright_white());
    println!("  Batch Size: {}", manifest.training.config.batch_size.to_string().bright_white());
    println!("  Grad Accumulation: {}", manifest.training.config.gradient_accumulation_steps.to_string().bright_white());
    println!();
    println!("  Dataset: {}", dataset_path.display().to_string().bright_white());
    ...
    println!("  {}", "Note: CUDA detection handled by Python training script".bright_blue());
    ...
    match result {
        Ok(_) => {
            ...
            println!();
            println!("{}", "✅ Training completed successfully!".bright_green().bold());
            println!();
            ...
            println!("  1. Validate: expert-cli validate --expert {}", output_dir.display());
            println!("  2. Package:  expert-cli package --manifest {} --weights {}", 
                     manifest_path.display(), output_dir.display());
            println!();
            Ok(())
        }
        Err(e) => {
