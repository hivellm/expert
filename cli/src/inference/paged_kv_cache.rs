...
impl PagedKVCache {
    pub fn new(config: PagedKVCacheConfig, device: Device) -> Result<Self> {
        ...
        let physical_pages = vec![
            Tensor::zeros(page_shape, config.dtype, &device)?
        ];
    ...
    pub fn allocate_page(&mut self, seq_id: u64) -> Result<usize> {
        ...
        if let Some(page_id) = self.free_pages.pop() {
            ...
            // Add to page table
            self.page_tables.entry(seq_id).or_insert_with(Vec::new).push(page_id);
    ...
    fn evict_page_lru(&mut self) -> Result<()> {
        ...
        let (oldest_page, _) = self.page_access_time
            .iter()
    ...
    pub fn read_kv(&mut self, seq_id: u64, _layer_idx: usize, seq_len: usize) -> Result<(Tensor, Tensor)> {
        let pages = self.page_tables.get(&seq_id).cloned()
            .ok_or_else(|| anyhow::anyhow!("Sequence {} not found", seq_id))?;
        
        if pages.is_empty() {
            return Err(anyhow::anyhow!("No pages allocated for sequence {}", seq_id));
        }
...
impl CacheStats {
    pub fn print(&self) {
        ...
        println!("Used pages:     {} ({:.1}%)", self.used_pages, 
