...
enum Commands {
    ...
    Chat {
        ...
        #[arg(
            long,
            help = "Temperature (0.0=greedy, 2.0=creative). Overrides manifest."
        )]
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Install { source, dev } => commands::install::install(&source, dev),
        Commands::List {
            verbose,
            base_model,
            models,
        } => commands::list::list(verbose, base_model, models),
        Commands::Uninstall { name, cleanup } => commands::uninstall::uninstall(&name, cleanup),
        Commands::Update { name, force } => commands::update::update(&name, force),
        Commands::Chat {
            experts,
            base_model,
            device,
...
fn print_banner() {
    ...
    println!(
        "{}",
        "║     HiveLLM Expert System CLI        ║".bright_blue()
    );
    println!(
        "{}",
        "║     Train • Package • Deploy         ║".bright_blue()
    );
    println!(
        "{}",
        "╚═══════════════════════════════════════╝".bright_blue()
