...
impl PythonTrainer {
    ...
    pub fn train_via_subprocess(
        ...
        let base_model_name = base_models
            .first()
        ...
        if !temp_config.exists() {
            return Err(crate::error::Error::Training(format!(
                "Failed to create temp config at: {}",
                temp_config.display()
            )));
        }
        ...
        cmd.env(
            "VIRTUAL_ENV",
            venv_python.parent().unwrap().parent().unwrap(),
        );

        let status = cmd.status().map_err(|e| {
            crate::error::Error::Training(format!("Failed to execute Python: {}", e))
        })?;
    ...
    pub fn train(
        ...
        Python::with_gil(|py| {
            ...
            let train_fn = trainer_module.getattr("train_expert").map_err(|e| {
                crate::error::Error::Training(format!("Failed to get train_expert function: {}", e))
            })?;
            ...
            let dataset_json = serde_json::to_string(&manifest.training.dataset).map_err(|e| {
                crate::error::Error::Training(format!("Failed to serialize dataset config: {}", e))
            })?;
            let dataset_dict: Bound<PyDict> = py
                .eval_bound(
                    &format!("__import__('json').loads(r'''{}''')", dataset_json),
                    None,
                    None,
                )?
                .extract()?;
