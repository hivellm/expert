...
fn test_package_v2_model_names_are_distinct() {
    ...
    assert!(
        qwen_06b_count >= 1,
        "Qwen3-0.6B should appear at least once"
    );
    assert!(
        qwen_15b_count >= 1,
        "Qwen3-1.5B should appear at least once"
    );
}
...
fn test_package_v2_adapters_embedded_in_models() {
    ...
    assert!(
        adapters_line > base_models_line,
...
fn test_package_v2_weight_paths_include_model_identifier() {
    ...
    assert!(
        content.contains("qwen3-0.6b"),
        "Path should include model identifier"
    );
    assert!(
        content.contains("qwen3-1.5b"),
        "Path should include model identifier"
    );
...
fn test_package_adapter_count_consistency() {
    ...
    assert_eq!(
        v2_adapter_arrays, 2,
        "v2.0 should have adapters arrays per model"
    );
}
...
fn test_json_formatting_validity() {
    ...
    let v1_json: serde_json::Value =
        serde_json::from_str(&v1_content).expect("v1.0 manifest is not valid JSON");
    let v2_json: serde_json::Value =
        serde_json::from_str(&v2_content).expect("v2.0 manifest is not valid JSON");
...
fn test_schema_version_field_presence() {
    ...
    assert_eq!(
        v1_json.get("schema_version").and_then(|v| v.as_str()),
        Some("1.0")
    );
    assert_eq!(
        v2_json.get("schema_version").and_then(|v| v.as_str()),
        Some("2.0")
    );
}
